...
def test_alignment(request):
	...
	if len(request.POST.get('fasta_string', False)) > 0:
		# Input formatting
	...
	#file_obj = file_up.save()
		...
		uploaded_filepath = temp_files + job_id + '.aln'
		...
		temp_dict = aln_to_dict('/Volumes/HDD/Projects/serverH/server/hivis/static/temp/uploads/769bfb')
		# End file upload bit

	
	# Come together here to pass to js
